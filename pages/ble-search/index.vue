...
const updateDisplayList = () => {
  ...
  if (query) {
    list = list.filter((device) => {
      ...
      const mac = resolveDeviceMac(device).toUpperCase();
...
const onQuery = async () => {
  ...
  try {
    ...
    await bleManager.startScan((device) => {
      ...
      const exists = allScannedDevices.value.some((d) => resolveDeviceMac(d) === mac);
      if (!exists) {
...
const handleConnect = async (device: any) => {
  ...
  try {
    // 2. 调用 Store 发起完整连接时序（包含自动状态及全局弹窗托管）
    const res = await bleStore.connectDevice(device.deviceId, deviceName, macAddress);

    // 3. 连接成功：根据是否早已连上做分流展示，确保用户知悉状态且体验流畅
    ...
    // 全局 Store 已自动捕获并展示错误弹层，此处无需额外操作
  }
};
