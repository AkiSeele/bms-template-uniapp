...
export const APP_CONFIG = {
  ...
  APP_MODE: "offline" as "cloud" | "offline",
  ...
  BLE_SCAN: {
    ...
    /**
     * 蓝牙扫描过滤的服务 UUID 列表
     * - 如果填入特定的主服务 UUID（如默认协议的 "00000001-0000-1000-8000-00805F9B34FB"），则只扫描广播该 UUID 的目标设备，以显著降低手机搜索功耗并过滤杂乱外设。
     * - 如果为空数组（[]）或为 undefined，则表示不过滤，扫描周边所有可见蓝牙外设。
     * - 微信小程序兼容性红线：若不过滤，底层接口的 services 参数必须不传或置为 undefined，绝对禁止传递空数组 []，否则在部分宿主系统下会被过滤判定为空导致扫描列表全空。
     */
    SCAN_SERVICES: [] as string[],
    /** 扫码/自动连接配对最大超时时间（毫秒） */
